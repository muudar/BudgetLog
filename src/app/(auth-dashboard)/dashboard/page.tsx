...
const page = async () => {
  ...
  return (
    <>
      <div className="grid max-w-[calc(100vw-32px)] grid-cols-2 grid-rows-2 gap-2 lg:col-span-6">
        <MainDashboardCards userData={userData}></MainDashboardCards>
      </div>
      <div className="flex w-full max-w-[calc(100vw-32px)] flex-col justify-center border py-4 md:px-4 lg:col-span-4 lg:h-[350px] xl:h-[400px]">
        <ChartDataWrapper></ChartDataWrapper>
      </div>
      <div className="max-w-[calc(100vw-32px)] rounded-md border lg:col-span-4">
        <RecentTransactions currency={userData.currency}></RecentTransactions>
      </div>
      <div className="flex h-full max-w-[calc(100vw-32px)] flex-col justify-between space-y-4 lg:col-span-6">
